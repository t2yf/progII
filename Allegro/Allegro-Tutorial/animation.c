//gcc animation.c -o animation $(pkg-config allegro-5 allegro_font-5 allegro_image-5 allegro_primitives-5 --libs --cflags)
...
/*definir tudo o que sera animado*/
enum BOUNCER_TYPE {
    BT_HELLO = 0,
    BT_JAX,
    BT_TRIANGLE,
...
int main()
{
    ...
    /*todo elemento a ser animado ganha uma posição aleatória e uma velocidade aleatória*/
    BOUNCER obj[BT_N];
    for(int i = 0; i < BT_N; i++)
    {
        ...
        b->x = rand() % 640;
        b->y = rand() % 480;
        b->dx = ((((float)rand()) / RAND_MAX) - 0.5) * 2 * 4; /*o quanto será movimentado a cada frame*/
    ...
    while(1)
    {
        ...
        switch(event.type)
        {
            case ALLEGRO_EVENT_TIMER:
                /*se movimentar a cada frame > batendo nas paredes se for a mais*/
                /*OBS seria legal considerar o tamanho total do objeto, para que ele respeite os limites da tela*/
                for(int i = 0; i < BT_N; i++)
                {
                    BOUNCER* b = &obj[i];
                    b->x += b->dx; /*se movimentar dx pixels*/
                    b->y += b->dy; /* se movimentar dy pixels*/
                    /*inverter a direção se bater na parede*/
                    if(b->x < 0)
                    {
                        b->x  *= -1;
                        b->dx *= -1;
        ...
        if(redraw && al_is_event_queue_empty(queue))
        {
            ...
            for(int i = 0; i < BT_N; i++)
            {
                ...
                switch(b->type)
                {   
                    /*atualizar frame desenhado, de acordo com as novas coordenadas do elemento (b->x e b->y)*/
                    case BT_HELLO:
                        al_draw_text(font, al_map_rgb(255, 255, 255), b->x, b->y, 0, "Hello world!");
                        break;

                    case BT_JAX:
                        al_draw_bitmap(jax, b->x, b->y, 0);
                        break;
...
//al_set_blender(ALLEGRO_ADD, ALLEGRO_ONE, ALLEGRO_ONE); >> fazer ficar transparente